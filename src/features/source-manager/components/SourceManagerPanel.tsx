 ...
 * Merges Current Live Source and Available Sources (Speaker Screen Shares & Materials) into a single unified list.
 * "Take Live" acts as "Go Live / Present", taking the source live to Audience & Confidence displays.
 */
    ...
    <div className="glass-panel p-4 sm:p-6 rounded-3xl border border-slate-800 space-y-4">
        ...
        <div className="flex items-center space-x-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Sumber presentasi & layar pembicara realtime yang siap ditayangkan di panggung.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 pt-1">
        {sources.map((source) => {
          ...
          const isScreenShare = source.type === "screen_share";
              ...
              <div>
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="flex items-center space-x-1.5 flex-wrap gap-y-1">
                    {isScreenShare ? (
                      ...
                      <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-purple-400 flex items-center space-x-1">
                ...
                {source.totalPages ? (
                  <p className="text-[10px] font-mono text-slate-400 truncate">
                    {source.totalPages} {source.totalPages === 1 ? "Slide" : "Slides"} • {source.mediaType?.toUpperCase() || "PRESENTATION"}
                  </p>
                ) : (
                  <p className="text-[10px] font-mono text-slate-400 truncate">
                ...
                <div className="pt-3 mt-3 border-t border-slate-800/80 flex items-center space-x-2">
                  {isLive ? (
                    ...
                    <button
                      ...
                      } active:scale-95 text-white font-bold text-xs transition flex items-center justify-center space-x-1.5 shadow-md cursor-pointer touch-manipulation`}
                    >
                      <Play className="w-3.5 h-3.5 fill-current pointer-events-none" />
                      <span className="pointer-events-none">
                        {isScreenShare ? "TAKE SCREEN LIVE" : "TAKE MATERIAL LIVE"}
                      </span>
                    </button>
                  )}
                </div>
